...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.10                                                                             #
# Filename   : /aimobile/scraper/appstore/database/base.py                                         #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
class Database(ABC):
    ...
    def replace(self, data: pd.DataFrame, tablename: str) -> int:
        """Inserts data in pandas DataFrame format into the designated table.
        ...
        """
        try:
            return data.to_sql(tablename, con=self._connection, if_exists="replace", index=False)
        except SQLAlchemyError as e:  # pragma: no cover
            msg = f"Exception occurred during database insert.\nException type:{type[SQLAlchemyError]}\n{e}"
            self._logger.error(msg)
            raise e
