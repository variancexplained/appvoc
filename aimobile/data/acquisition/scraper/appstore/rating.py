...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.10                                                                             #
# Filename   : /aimobile/data/acquisition/scraper/appstore/rating.py                               #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
# Modified   : Saturday April 29th 2023 11:16:02 am                                                #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
# ------------------------------------------------------------------------------------------------ #
@dataclass
class AppStoreRatingRequest(Request):
    id: int
    url: str = None
    params: Dict[str:str] = field(default_factory=dict)
    headers: Dict[str:str] = STOREFRONT
    created: datetime = datetime.now()


# ------------------------------------------------------------------------------------------------ #
@dataclass
class AppStoreRatingResult(Result):
    id: int
    controller: str = None
    term: str = None
...
# ------------------------------------------------------------------------------------------------ #
class AppStoreRatingRequestGenerator(RequestGenerator):
...
# ------------------------------------------------------------------------------------------------ #
class AppStoreRatingParser(ResponseParser):
    """Parses the response from an requests.Response object"""

    def __init__(self, response: requests.Response) -> None:
        self._response = response

    def parse_response(self, request: Request, response: requests.Response) -> AppStoreRatingResult:
        ...
        if response.status_code == 200:
            ...
            return AppStoreRatingResult(request=request, valid=True, size=len(df), content=df)
        else:
            msg = f"Invalid response encountered. Status code = {response.status_code}"
            self._logger.error(msg)
            return AppStoreRatingResult(request=request, valid=False, size=0, content=None)
...
class AppStoreRatingController(Controller):
    ...
    def create_generator(self) -> RequestGenerator:
        ...
        page = self._get_progress()
        generator = AppStoreRatingRequestGenerator(uow=self._uow, term=self._term, page=page)
    ...
    def parse_response(self) -> Result:
        ...
        self._result = AppStoreRatingParser(self._request, self._response)
