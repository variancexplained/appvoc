...
# Version    : 0.1.0                                                                               #
# Python     : 3.10.10                                                                             #
# Filename   : /aimobile/data/scraper/appstore/repo/review.py                                      #
# ------------------------------------------------------------------------------------------------ #
# Author     : John James                                                                          #
...
class AppStoreReviewRepo(Repo):
    ...
    def get(self, category_name: str) -> pd.DataFrame:
        ...
        """
        query = "SELECT * FROM review WHERE category_name = ?;"
    ...
    def add(self, data: pd.DataFrame) -> None:
        ...
        """
        self._database.create(data=data, tablename="review")
    ...
    def remove(self, category_name: str = None, project_id: str = None) -> None:
        ...
        if category_name is not None and project_id is not None:
            query = "DELETE FROM review WHERE category_name = ? AND project_id = ?;"
            params = (
                category_name,
                project_id,
            )
        elif category_name is not None:
            query = "DELETE FROM review WHERE category_name = ?;"
            params = (category_name,)
        elif project_id is not None:
            query = "DELETE FROM review WHERE project_id = ?;"
            params = (project_id,)
        else:
