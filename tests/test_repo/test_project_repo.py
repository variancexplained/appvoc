...
# Modified   : Saturday May 20th 2023 01:13:11 pm                                                  #
# ------------------------------------------------------------------------------------------------ #
# License    : MIT License                                                                         #
...
class TestAppDataProject:  # pragma: no cover
    ...
    def test_from_df(self, caplog):
        ...
        assert isinstance(project.completed, datetime)
        # ---------------------------------------------------------------------------------------- #
        end = datetime.now()
        duration = round((end - start).total_seconds(), 1)
    ...
    def test_update(self, appdata_project_repo, caplog):
        ...
        for i in range(10):
            PROJECT.update(i)
            pages = i
            assert PROJECT.pages == pages
            assert isinstance(PROJECT.updated, datetime)
            assert dt != PROJECT.updated
            dt = PROJECT.updated
    ...
    def test_complete(self, caplog):
        ...
        PROJECT.complete()
        assert PROJECT.status == "complete"
        assert PROJECT.started < PROJECT.completed
    ...
    def test_as_dict(self, caplog):
        ...
        assert d["controller"] == "TestController"
        assert d["term"] == "social"
...
class TestAppDataProjectRepo:  # pragma: no cover
    ...
    def test_replace(self, appdata_project_repo, caplog):
        ...
        df = repo.getall()
        assert df.shape[0] == 1
        assert df["controller"][0] == "ReplacedController"
    ...
    def test_get_project(self, appdata_project_repo, caplog):
        ...
        assert project.controller == "ReplacedController"
        assert project.term == "social"
    ...
    def test_update(self, appdata_project_repo, caplog):
        ...
        for i in range(10):
            PROJECT.update(i)
            repo.update(data=PROJECT)
            repo.save()
        project = repo.get_project(controller="ReplacedController", term="social")
        logger.debug(project)
        assert project.pages == 10
